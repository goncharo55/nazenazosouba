...
```json
{
  ...
  "tags": ["#タグ1", "#タグ2"],
  "indices": [
    {"ticker": "^GSPC", "name": "S&P500", "value_text": "7,691.76", "unit": "ポイント",
...
- `indices`（Market Snapshotのカード）は **12件** を目安に、以下の順で並べる
  ...
  - `unit` は数値の後ろに小さく添える単位（`"円"` `"ポイント"` `"%"` `"ドル"` など）。米国株指数は
    円やドルではなく無次元の指数値なので `"ポイント"`、日経平均やTOPIX(ETF)・ドル円は `"円"`、
    金利は `"%"`、金・原油・ビットコインなど米ドル建て資産は `"ドル"`。
